...
int TeleSocket::createReceiveServer(const int port, std::vector<message_buf>& message)
{
	...
	//写日志
	string logSql = "insert into 系统日志表 (时间,对象,事件类型,事件说明) values (now(),'遥测通信模块',11000,'建立TCP连接');";
	...
	while (true) {
		...
		while (1) {
			...
			if (retVal == 0) {
				...
				//写日志
				logSql = "insert into 系统日志表 (时间,对象,事件类型,事件说明) values (now(),'遥测通信模块',11001,'断开TCP连接');";
				mysql.writeDataToDB(logSql);
				closesocket(sServer);   //关闭套接字    
				closesocket(sClient);   //关闭套接字
				return -1;
